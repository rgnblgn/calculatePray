...
export default function MainScreen({ debtDate, onReset }: MainScreenProps) {
    ...
    const [totalVoluntaryPrayed, setTotalVoluntaryPrayed] = useState<number>(0);
    ...
    const loadInitialData = async () => {
        ...
        if (storedData) {
            ...
            setTotalVoluntaryPrayed(storedData.totalVoluntaryPrayed || 0);
        }
    };
    ...
    const saveCurrentData = useCallback(async () => {
        await saveData({
            ...
            voluntaryPrayers,
            paidDebts,
            totalVoluntaryPrayed,
        });
    }, [debtDate, currentDebts, voluntaryPrayers, paidDebts, totalVoluntaryPrayed]);

    // Counter değişikliklerini otomatik kaydet
    useEffect(() => {
        saveCurrentData();
    }, [currentDebts, voluntaryPrayers, paidDebts, totalVoluntaryPrayed, saveCurrentData]);
    ...
    const handleSaveVoluntaryPrayers = async () => {
        ...
        // Toplam kılınan nafile namazları güncelle
        const totalVoluntaryCount = Object.values(voluntaryPrayers).reduce(
            (sum, count) => sum + count,
            0
        );
        const updatedTotalVoluntary = totalVoluntaryPrayed + totalVoluntaryCount;

        // State'leri güncelle - useEffect otomatik kaydedecek
        setPaidDebts(updatedPaidDebts);
        setTotalVoluntaryPrayed(updatedTotalVoluntary);
    ...
    // Gün hesabı: Her vakitten (paidDebts) kaç tane kılınmışsa
                                ...
                                <>
                                    ...
                                    {showDebtDetails && remainingDays > 0 && (
                                        ...
                                        >
                                            ...
                                            {totalVoluntaryPrayed > 0 && (
                                                <>
                                                    <View style={styles.divider} />
                                                    <View
                                                        style={[
                                                            styles.voluntaryTotalContainer,
                                                            isDark && styles.voluntaryTotalContainerDark,
                                                        ]}
                                                    >
                                                        <ThemedText style={styles.voluntaryTotalIcon}>
                                                            🌟
                                                        </ThemedText>
                                                        <View style={{ flex: 1 }}>
                                                            <ThemedText
                                                                style={[
                                                                    styles.voluntaryTotalText,
